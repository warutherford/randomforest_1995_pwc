# 1995 Predicted Woody Cover and Biophysical Variable Random Forest
# Austin Rutherford
...
# Set negative 1995 NDVI Landsat predicted woody cover (pwc) values to 0,
...
pwc_clean <- pwc_data %>%
  mutate(clay = na_if(clay, -9999),
         ...
         pwc_ninefive = replace(pwc_ninefive, which(pwc_ninefive<0),NA),
         pwc_ninefive = pwc_ninefive/100,
...
# Preprocess data
pwc_rec <- recipe(pwc_ninefive ~ ., data = pwc_train) %>%
...
pwc_rec_small <- recipe(pwc_ninefive ~ point + easting + northing + elevation + clay + aspect + fallsum + fallmean + wintersum + wintermean + slope + washdist, data = pwc_train) %>%
...
# Save an RDS file for future use if needed, save that memory
saveRDS(rf_fit_small, "./randforest_rds/first_model_RF_small_1995.rds")
...
ggsave('Graphs/rf_param_plot_small_1995.png', plot = rf_param_plot_small)
...
# Pull out parameters of the best model based on RMSE (for prediction), mtry = 5, min_n = 2
...
pwc_vip_small <- final_rf_small %>%
  ...
  fit(pwc_ninefive ~ elevation + clay + aspect + fallsum + fallmean + wintersum + wintermean + slope + washdist,
...
ggsave('Graphs/pwc_vip_9_1995.png', plot = pwc_vip_small)
...
saveRDS(final_res_small, "./randforest_rds/final_model_RF_small_1995.rds")

# Compare predicted pwc values to test pwc
final_res_small %>% 
  collect_predictions(summarize = FALSE) %>% arrange(desc(pwc_ninefive))

# graph of predicted values to testing pwc
rf_plot_small <- final_res_small %>%
  collect_predictions() %>%
  ggplot(aes(100*pwc_ninefive, 100*.pred)) +
  geom_point(size = 0.5, alpha = 0.5) +
  labs(y = '1995 Random Forest Predicted Woody Cover (%)', x = '1995 Landsat NDVI Predicted Woody Cover (%)') +
  ylim(c(50, 100))+
  xlim(c(50, 100))+
  scale_color_manual(values = c("gray80", "darkred"))+
  geom_abline(intercept = 0, slope = 1, color = "blue", size = 1)+
...
ggsave('Graphs/RF_Model_Small_1995.png', plot = rf_plot_small)


# Apply random forest model to create predictions on full raster

# # Load RDS file if need to shut down R
#rf_fit_small_last <- readRDS("./randforest_rds/final_model_RF_small_1995.rds")
...
# Save the RDS file for further use, save that memory
saveRDS(rf_mod_pred, "./randforest_rds/pred_model_RF_small_1995.rds")
...
pwc_small_rf_comb %>% 
  ggplot(aes(100*pwc_ninefive, 100*predicted_pwc)) +
  geom_point(size = 0.5, alpha = 0.5) +
  labs(y = '1995 Random Forest Predicted Woody Cover (%)', x = '1995 Landsat NDVI Predicted Woody Cover (%)') +
  ylim(c(50, 100))+
  xlim(c(50, 100))+
...
write_csv(pwc_small_rf_comb, path = "Data/rf_model_pred_1995.csv")
